...
import 'package:kologsoft/screens/stockrequest.dart';
...
class StockRequestDetails extends StatefulWidget {
  const StockRequestDetails({super.key});
  @override
  State<StockRequestDetails> createState() => _StockRequestDetailsState();
}

class _StockRequestDetailsState extends State<StockRequestDetails> {
              ...
              child:  ListView(
                children: [
                  ...
                  StreamBuilder<List<Map<String, dynamic>>>(
                    stream: Provider.of<StockProvider>(context, listen: false).stockStream(collectionName: 'stock_request',startDate: _isDateRangeActive ? _startDate : null, endDate: _isDateRangeActive ? _endDate : null, branchfield: 'branchid', datefield: 'createdAt',),
                                ...
                                ...items.map((item) {
                                  return Column(
                                    ...
                                    children: [
                                      ...
                                      Text("Qty: ${(item['requestedquantity'] as num?)?.toDouble() ??0.0 }", style: const TextStyle(color: Colors.white)),
                                      Text("Mode: ${item['mode']}", style: const TextStyle(color: Colors.white)),
                                            ...
                                            onPressed: () async {
                                              final confirm = await showDialog<bool>(
                                                ...
                                                builder: (_) => AlertDialog(
                                                  title: Text("Delete Request Invoice"),
                                                  content: const Text("Are you sure you want to delete this request invoice?"),
                                                  actions: [
                                                    TextButton(
                                              ...
                                              if (confirm == true) {
                                                await db.collection('stock_request').doc(doc['docid']).delete();
                                                if (context.mounted) {
                                                  ScaffoldMessenger.of(context).showSnackBar(
                                                    const SnackBar(content: Text("Stock request deleted successfully"),backgroundColor: Colors.green,),
                                                  );
                                                }
                                              }
                                            },
