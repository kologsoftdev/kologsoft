...
class StockBalanceSync extends StatefulWidget {
  const StockBalanceSync({super.key});

  @override
  State<StockBalanceSync> createState() => _StockBalanceSyncState();
}

class _StockBalanceSyncState extends State<StockBalanceSync> {
  // ── Single-item fields
  ...
  Future<void> _syncBalance() async {
    ...
    try {
      ...
      await provider.db
          ...
          .update({
        'branchbalance.$_selectedBranchId.netpieces': balance ?? 0,
        'branchbalance.$_selectedBranchId.lastupdate':
        FieldValue.serverTimestamp(),
      });
      setState(() {
        _resultMessage =
        'Synced  ${_selectedItem!.name}  →  $_selectedBranchName  |  ${balance ?? 0} pcs';
        _resultSuccess = true;
      });
    } catch (e) {
                            ...
                            children: [
                              ...
                              DropdownButtonFormField<String>(
                                ...
                                onChanged: _syncing
                                  ...
                                  if (_tab == 0) {
                                    // Single item tab: do nothing, wait for user to select item
                                  } else if (_tab == 1) {
