...
Future<void> generateInvoicePdf(Map<String, dynamic> data, List<Map<String, dynamic>> items) async {
  ...
  final selecteddate = (data['transferdate'] as Timestamp).toDate();

  // Calculate totals with proper type casting
  final grossTotal = items.fold<double>(0.0, (sum, item) {
  ...
  final itemCount = items.fold<int>(0, (int sum, item) => sum + ((item['quantity'] as num?)?.toInt() ?? 0));
