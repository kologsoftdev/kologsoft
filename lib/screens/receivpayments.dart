...
class ReceivepaymentsPage extends StatefulWidget {
  ...
  @override
  State<ReceivepaymentsPage> createState() => _ReceivepaymentsPageState();
}
...
class _ReceivepaymentsPageState extends State<ReceivepaymentsPage> {
  ...
  Widget _buildEmptyState(CashierProvider provider) {
    final hasData = provider.filteredDebtors.isNotEmpty;
    ...
    return Center(
      child: Column(
        ...
        children: [
          Icon(
            hasData ? Icons.search : Icons.people_outline,
            size: 64,
  ...
  Widget _buildSingleDebtorCard(CashierProvider provider) {
    final debtor = _selectedDebtor!;

    return SingleChildScrollView(
      padding: const EdgeInsets.all(12),
      child: DebtorDetailCard(
        debtor: debtor,
        onReceivePayment: () {
  ...
  void _showDebtorDetails(BuildContext context, Debtor debtor) async {
    ...
    await provider.loadDebtorDetails(customerid: debtor.id);
    showModalBottomSheet(
      context: context,
      isScrollControlled: true,
      backgroundColor: Colors.transparent,
...
class DebtorDetailCard extends StatelessWidget {
  final Debtor debtor;
  final VoidCallback onReceivePayment;
  final VoidCallback onViewDetails;
  final VoidCallback onBack;

  const DebtorDetailCard({
    super.key,
          ...
          children: [
            ...
            Row(
              children: [
                ...
                Container(
                  ...
                  child: Text(
                    debtor.status,
                    style: TextStyle(color: debtor.statusColor, fontSize: 14, fontWeight: FontWeight.w600),
                  ),
                ),
              ],
            ),
            ...
            Container(
              ...
              child: Row(
                children: [
                  Expanded(
                    child: _buildDetailColumn('Total Debt', 'GHS ${NumberFormat('#,##0.00').format(debtor.creditBalance)}', Colors.white),
                  ),
                  Container(
                    height: 40,
                    width: 1,
                  ...
                  Expanded(
                    child: _buildDetailColumn('Amount Paid', 'GHS ${NumberFormat('#,##0.00').format(debtor.amountpaid)}', Colors.green.shade400),
                  ),
                  Container(
                    height: 40,
                    width: 1,
            ...
            Container(
              ...
              child: Column(
                ...
                children: [
                  ...
                  const SizedBox(height: 8),
                  _buildInfoRow('Credit Limit', debtor.creditLimit.isEmpty ? 'N/A' : 'GHS ${debtor.creditLimit}'),
                  const SizedBox(height: 8),
                  _buildInfoRow('Branch', debtor.branchName.isEmpty ? 'N/A' : debtor.branchName),
                ],
              ),
            ),
              ...
              children: [
                Expanded(
                  child: ElevatedButton.icon(
                    ...
                    label: const Text('Receive Payment', style: TextStyle(fontSize: 16)),
                    style: ElevatedButton.styleFrom(
                      backgroundColor: Colors.blue.shade700,
                      foregroundColor: Colors.white,
  ...
  Widget _buildDetailColumn(String label, String amount, Color color) {
    return Column(
